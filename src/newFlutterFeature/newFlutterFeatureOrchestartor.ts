...
export const newFlutterFeatureOrchestrator = async (uri: Uri) => {
  ...
  const rawFeatureType: string | undefined = await promptForOptionsInput(
    [FlutterFeatureType.full, FlutterFeatureType.presentation],
  ...
  const featureType: FlutterFeatureType =
    FlutterFeatureType[rawFeatureType as keyof typeof FlutterFeatureType];

  const featureClassName = titleCase(featureName);
  const snakeCaseFeatureName = snakeCase(featureName);

  try {
    await generateFlutterDirectories(
